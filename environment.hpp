...
Expressions as arguments and returning an Expression.
*/
typedef Expression(*SpecialProc)(const std::vector<Expression> & args, Environment & env);
...
public:
  ...
  */
  ...
  /*! Remove a mapping from sym argument to the exp argument within the environment.
  \param sym the symbol to remove
  \param exp the expression the symbol should map to
  */
  void rm_exp(const Atom &sym);
  ...
  */
  ...
  /*! Get the Special Procedure the argument symbol maps to
  \param sym the symbol to lookup
  \return the special procedure it maps to
  */
  SpecialProc get_spec(const Atom &sym) const;

  /*! Determine if a symbol has been defined as a lambda procedure
  \param sym the symbol to lookup
  \return true if the symbol maps to a lambda procedure
  */
  bool is_lamb(const Atom &sym) const;

  /*! Get the Lambda the argument symbol maps to
  \param sym the symbol to lookup
  \return the procedure it maps to
  */
  Expression get_lamb(const Atom & sym) const;

  /*! Add a mapping from sym argument to the lamb argument within the environment.
  \param sym the symbol to add
  \param lamb the lambda procedure the symbol should map to
  */
  void add_lamb(const Atom & sym, const Expression & exp);  
...
private:
  ...
  enum EnvResultType { ExpressionType, ProcedureType, SpecialType, LambdaType };

  struct EnvResult {
    EnvResultType type;
    Expression exp; // used when type is ExpressionType
    Procedure proc; // used when type is ProcedureType
	SpecialProc spec; //used when type is SpecialType
	...
	EnvResult(EnvResultType t, SpecialProc s) : type(t), spec(s) {};
  };
